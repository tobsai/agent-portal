...
// ---------------------------------------------------------------------------
// Minimal ChannelPlugin interface (mirrors OpenClaw's internal contract)
// Replace with import once the package is published to the OpenClaw registry.
// ---------------------------------------------------------------------------
...
async function resolveChannelId(
  ...
  if (!agentId || agentId === sessionKey) {
    throw new Error(
      `[portal-plugin] Cannot derive agentId from sessionKey "${sessionKey}". ` +
  ...
  const parsed = JSON.parse(data) as { id: string; is_dm?: boolean; dm_agent_id?: string };
  if (!parsed?.id) {
...
function httpPost(url: string, apiKey: string, body: Record<string, unknown>): Promise<string> {
  return new Promise((resolve, reject) => {
    const parsed = new URL(url);
    const lib = parsed.protocol === 'https:' ? https : http;
...
class AgentPortalChannelPlugin implements ChannelPlugin {
  readonly channelId = 'portal';
  ...
  async send(message: OutboundMessage): Promise<void> {
    ...
    try {
      await httpPost(
        `${this.portalUrl}/api/channels/${channelId}/messages`,
        this.apiKey,
        body,
      );
      console.log(
        `[portal-plugin] send() → channel ${channelId}` +
  ...
  private startWebhookServer(
    handler: (text: string, sessionKey: string) => void,
  ): http.Server {
    if (this.webhookServer) {
    ...
    const server = http.createServer((req, res) => {
      ...
      req.on('end', () => {
        ...
        try {
          event = JSON.parse(rawBody) as ChannelEvent;
        } catch {
        ...
        console.log(
          `[portal-plugin] webhook ← inbound (sessionKey=${event.sessionKey}, ` +
    ...
    server.on('error', (err: NodeJS.ErrnoException) => {
      if (err.code === 'EADDRINUSE') {
        console.error(
          `[portal-plugin] webhook port ${this.webhookPort} already in use. ` +
   ...
   */
  private verifySignature(body: string, signature: string | string[] | undefined): boolean {
 ...
 */
export function createPlugin(config: PluginConfig): ChannelPlugin {
