...
module.exports = function createChatGateway({ chatState, broadcast, sendAgentMessage, gatewayClient }) {
  ...
  function connectChatGateway() {
    ...
    if (!rawGwUrl) return;
    const gwUrl = normalizeGatewayWsUrl(rawGwUrl);

    chatGatewayWs = new WebSocket(gwUrl, { headers: { Origin: resolveGatewayOrigin(gwUrl) } });
    ...
    chatGatewayWs.on('message', async (data, isBinary) => {
      ...
      if (msg.event === 'chat') {
        console.log('[chat-gateway] raw event:', JSON.stringify(msg));
        const payload = msg.payload || msg.data || {};
        const state = payload.state;
        // Determine which agent this event is from; prefer explicit sessionKey in payload
        const lastActiveSessionKey = getLastActiveSessionKey();
        const eventSessionKey = payload.sessionKey || payload.session_key || lastActiveSessionKey;
        ...
        if (normalized) {
          ...
          // Tag with sessionKey so frontend and channel routing use the correct session
          normalized.sessionKey = eventSessionKey;
          ...
          if (normalized.role === 'assistant' && normalized.text) {
            ...
            const resolvedChannelId = sessionChannelMap.get(eventSessionKey) || lastActiveChannelId;
            sendAgentMessage(resolvedChannelId, normalized.text, agentName, agentEmoji, agentId).catch(() => {});
          }
        }
      }
    });
  ...
  function wireGatewayClientEvents() {
    ...
    gatewayClient.on('message', async (event) => {
      ...
      // Tag with sessionKey so frontend can filter by active channel
  ...
  /**
   * Attach the gateway WebSocket proxy connection handler to a ws.Server.
   * Handles server-side auth so browser clients don't need device identity.
   *
   * @param {import('ws').Server} gwProxy
   */
  function setupGwProxy(gwProxy) {
    gwProxy.on('connection', (clientWs, req) => {
      const gwUrl = normalizeGatewayWsUrl(process.env.GATEWAY_WS_URL || '');
      if (!gwUrl) { clientWs.close(1008, 'Gateway not configured'); return; }
      console.log('[gw-proxy] new client, connecting to:', gwUrl);
      const gwWs = new WebSocket(gwUrl, { headers: { Origin: resolveGatewayOrigin(gwUrl) } });
