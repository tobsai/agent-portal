...
/**
 * Check Sentry configuration (no-crash if unconfigured).
 * Returns 'ok' if SENTRY_DSN is set, 'unconfigured' otherwise.
 * Does NOT attempt a live connection.
 */
...
async function runHealthCheck({ db, gatewayClient } = {}) {
  ...
  // 'sentry' is informational only — 'unconfigured' does not affect overall status.
  const aggregatedChecks = Object.entries(checks)
    .filter(([key]) => key !== 'sentry')
    .map(([, val]) => val.status);
  const overallStatus = aggregatedChecks.includes('down')     ? 'down'
                      : aggregatedChecks.includes('degraded') ? 'degraded'
                      :                                         'ok';
...
module.exports = { runHealthCheck, checkDb, checkGateway, checkApns, checkScheduledTasks, checkSentry };
