...
const GATEWAY_URL = process.env.GATEWAY_WS_URL_LOCAL || 'ws://127.0.0.1:18789';
...
class GatewayClient extends EventEmitter {
  ...
  _onMessage(text) {
    ...
    if (msg.type === 'res' && typeof msg.id === 'string' && msg.id.startsWith('gw-connect-')) {
      if (msg.ok) {
        ...
        this.emit('error', new Error(msg.error?.message || 'Gateway auth failed'));
  ...
  _handleChallenge(nonce) {
    ...
    const params = {
      minProtocol: 3,
      maxProtocol: 3,
      client: { id: 'portal-server', version: '1.0.0', platform: 'server', mode: 'channel' },
      role: 'operator',
    ...
    if (privateKeyPem && publicKeyB64) {
      try {
        ...
        const payload = ['v2', deviceId, 'portal-server', 'channel', 'operator', scopes, String(signedAt), token, nonce].join('|');
