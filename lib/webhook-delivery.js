 ...
 */
async function deliverInboundWebhook({ channelId, sessionKey, text, senderId, timestamp }) {
  ...
  try {
    ...
    if (status >= 500) {
      ...
      await postOnce(webhookUrl, body, signature);
    }
  } catch (err) {
    // Swallow — webhook failure must not break message delivery
    console.error('[webhook] inbound delivery failed:', err.message);
  }
}
